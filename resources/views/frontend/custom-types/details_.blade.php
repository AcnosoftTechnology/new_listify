                        ...
                        <div class="detailstop-title-location1">
                            ...
                            <div class="location d-flex align-items-center">
                               <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                @php
                                    $city_name = App\Models\City::where('id', $listing->city)->first()?->name ?? 'N/A';
                                    $country_name = App\Models\Country::where('id', $listing->country)->first()?->name ?? 'N/A';                                    
                                @endphp
                                <p class="name"> {{ $city_name . ', ' . $country_name . ',' . $listing->address }} </p>
                            </div>
                        </div>
        ...
        <div class="container">
            ...
            @endif
               ...
               @php
                ...
                $invoices = DB::table('invoices')->where('user_id', $user_id)->get();

                $validInvoices = $invoices->filter(function($invoice) {
                    return !empty($invoice->note) 
                        || !empty($invoice->terms_condition) 
                        || !empty($invoice->shipping) 
                        || !empty($invoice->disclaimer);
                });
            @endphp
            ...
            {{-- Invoice Policy Section Addon --}}                 
                    ...
                    <div class="restdetails-agent-details mt-30px mb-50px">
                        <div class="restdetails-agent-header mb-16 d-flex align-items-center justify-content-between flex-wrap">
                            ...
                            <div class="restdetails-agent-btns d-flex align-items-center flex-wrap">
                                ...
                                <a href="{{ route('user.profile', ['slug' => Str::slug($user_name->name) . '-' . $user_name->id]) }}" class="theme-btn1 follow-btn">{{ get_phrase('View Profile') }}</a>
                            </div>
                        </div>
    ...
    <section>
        <div class="container">
            ...
            <div class="row row-28 mb-80">
                ...
                @endphp
                @foreach (sort_listings_by_distance($relatedListing) as $listings)
                    <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6" data-listing-id="{{ $listings->id }}" data-listing-type="{{ $type }}">
                         <div class="single-grid-card htd-grid-card">
                            <!-- Banner Slider -->
                            <div class="grid-slider-area">
                               @php
    $images = json_decode($listings->image, true) ?? [];
                            ...
                            <div class="hotel-grid-details position-relative">
                                ...
                                <div class="hotelgrid-location-rating d-flex align-items-center justify-content-between flex-wrap">
                                    <div class="location d-flex">
                                        ...
                                        @php
                                            $city_name = App\Models\City::where('id', $listings->city)->first()?->name ?? 'N/A';
                                            $country_name = App\Models\Country::where('id', $listings->country)->first()?->name ?? 'N/A';
                                        @endphp
                                        <p class="name"> {{ $city_name . ', ' . $country_name }} </p>
                                        @include('frontend.partials.listing_distance', ['listing' => $listings])
                                    </div>
                                    @php
