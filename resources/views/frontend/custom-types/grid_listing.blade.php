    ...
    <div class="container">
        <div class="row">
            <div class="col-12">
                ...
                @if(!empty($CustomlistingSetting->listing_type))
                    <p class="dm-uppercase-text-16px text-center">
                        {{ $CustomlistingSetting->listing_type }}
                    </p>
                @endif

                @if(!empty($CustomlistingSetting->banner_title))
                    <h1 class="dm-title-60px text-capitalize fw-semibold text-white mb-30px text-center">
                        {{ $CustomlistingSetting->banner_title }}
                    </h1>
                @endif

                @if(!empty($CustomlistingSetting->banner_description))
                    <p class="in-subtitle-16px text-white text-center max-w-723px mx-auto mb-40px">
                        {{ $CustomlistingSetting->banner_description }}
                    </p>
                @endif
                          ...
                          @if (count($listings) > 0)
                              <div class="row row-28">
                                  ...
                                  @foreach ($listings as $listing)
                                      ...
                                      <!-- Single Card -->
                                      <div class="col-sm-6 {{ get_frontend_settings('map_position') == 'right' ? 'col-sm-6' : 'col-xl-4' }} map-card" data-listing-id="{{ $listing->id }}" data-listing-type="{{ $type }}">
                                          ...
                                          <div class="single-grid-card">
                                              ...
                                              <div class="grid-slider-area">
                                                  ...
                                                  <a class="w-100 h-100"
                                                     href="{{ route('listing.details',['type'=>$type, 'id'=>$listing->id, 'slug'=>slugify($listing->title)]) }}">

                                                      <img class="card-item-image"
                                                           src="{{ $image ? get_all_image('listing-images/' . $image) : asset('assets/frontend/images/default.png') }}">

                                                  </a>

                                                  <p class="card-light-text theme-light capitalize">
                                                      {{ $listing->is_popular }}
                                                  </p>
                                                  ...
                                                  <a href="javascript:void(0);"
                                                     ...
                                                     onclick="updateWishlist(this, '{{ $listing->id }}')"
                                                     class="grid-list-bookmark white-bookmark {{ $is_in_wishlist ? 'active' : '' }}">

                                                      <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                           xmlns="http://www.w3.org/2000/svg">
                                                          <path d="M13.4361 3C12.7326 3.01162 12.0445 3.22023 11.4411 3.60475C10.8378 3.98927 10.3407 4.53609 10 5.18999C9.65929 4.53609 9.16217 3.98927 8.55886 3.60475C7.95554 3.22023 7.26738 3.01162 6.56389 3C5.44243 3.05176 4.38583 3.57288 3.62494 4.44953C2.86404 5.32617 2.4607 6.48707 2.50302 7.67861C2.50302 10.6961 5.49307 13.9917 8.00081 16.2262C8.56072 16.726 9.26864 17 10 17C10.7314 17 11.4393 16.726 11.9992 16.2262C14.5069 13.9917 17.497 10.6961 17.497 7.67861C17.5393 6.48707 17.136 5.32617 16.3751 4.44953C15.6142 3.57288 14.5576 3.05176 13.4361 3Z"
                                                                fill="#6C1CFF" />
                                                      </svg>
                                              ...
                                              <div class="hotel-grid-details position-relative">
                                                  ...
                                                  <a href="{{ route('listing.details', ['type' => $type, 'id' => $listing->id, 'slug' => slugify($listing->title)]) }}"
                                                     class="title">
                                                      ...
                                                      @if(isset($claimStatus) && $claimStatus->status == 1)
                                                          ...
                                                          <span data-bs-toggle="tooltip"
                                                              ...
                                                              <svg fill="none" height="18" viewBox="0 0 24 24" width="18"
                                                                   xmlns="http://www.w3.org/2000/svg">
                                                                  <linearGradient id="paint0_linear_16_1334" gradientUnits="userSpaceOnUse"
                                                                                  x1="12" x2="12" y1="-1.2" y2="25.2">
                                                                  ...
                                                                  <path d="m3.783 2.826 8.217-1.826 8.217 1.826c.2221.04936.4207.17297.563.3504.1424.17744.22.39812.22.6256v9.987c-.0001.9877-.244 1.9602-.7101 2.831s-1.14 1.6131-1.9619 2.161l-6.328 4.219-6.328-4.219c-.82173-.5478-1.49554-1.2899-1.96165-2.1605-.46611-.8707-.71011-1.8429-.71035-2.8305v-9.988c.00004-.22748.07764-.44816.21999-.6256.14235-.17743.34095-.30104.56301-.3504zm8.217 10.674 2.939 1.545-.561-3.272 2.377-2.318-3.286-.478-1.469-2.977-1.47 2.977-3.285.478 2.377 2.318-.56 3.272z"
                                                                        fill="url(#paint0_linear_16_1334)"/>
                                                              </svg>
                                                  ...
                                                  <div class="hotelgrid-see-price d-flex align-items-center justify-content-between">
                                                      ...
                                                      <a href="{{ route('listing.details', ['type' => $type, 'id' => $listing->id, 'slug' => slugify($listing->title)]) }}"
                                                         class="see-details-btn1 stretched-link">
        ...
        <div class="row justify-content-center mb-100px">
            <div class="col-xl-12">
                ...
                <div class="bg-img-card2"
                    style="background-image: url('{{ asset($CustomlistingSetting->cta_bg_image) }}');
                           background-repeat: no-repeat;
                           width: 100%;
                           background-position: center;
                           background-size: cover;">

                    @if(!empty($CustomlistingSetting->cta_bg_title))
                        <h1 class="dm-title-36px mb-26px text-white">
                            {{ $CustomlistingSetting->cta_bg_title }}
                        </h1>
                    @endif

                    @if(!empty($CustomlistingSetting->cta_bg_description))
                        <p style="color:#fff; font-size:16px; margin-bottom:20px; font-weight:500;">
                            {{ $CustomlistingSetting->cta_bg_description }}
                        </p>
                    @endif

                    @if(!empty($CustomlistingSetting->cta_tab_name) && !empty($CustomlistingSetting->cta_tab_link))
                        <div class="d-flex align-items-center gap-14px flex-wrap">
                            <a href="{{ $CustomlistingSetting->cta_tab_link }}" class="btn ht-btn-white">
                                {{ $CustomlistingSetting->cta_tab_name }}
                            </a>
                        </div>
                    @endif
