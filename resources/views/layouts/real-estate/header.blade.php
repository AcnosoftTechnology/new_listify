@if (request()->is('real-estate'))
    ...
    <div class="real-header-banner-wrap mb-100px" style="background-image: url('{{ isset($RealEstateBanner->image) && $RealEstateBanner->image ? asset('uploads/homepage/real-estate/' . $RealEstateBanner->image) : asset('assets/frontend/images/real-estate/real-estate-banner.webp') }}');">
        ...
        <!-- Start Header -->
        <header class="header-section bg-light" style="margin-top:-20px;">
            <div class="container">
 <div class="row">
            <div class="col-md-12">
                <div class="main-header-area d-flex align-items-center justify-content-between">
                    <div class="header-logo">
                        <a href="{{ route('home') }}" style="width: 180px;">
...
<ul class="header-nav-list">
    @php
        ...
        $customSlugs = App\Models\CustomlistingSetting::pluck('slug')->toArray();
    @endphp
    ...
    @foreach ($menu_items as $index => $item)
        @php
            ...
            if (in_array($slug, $customSlugs)) {
                // Custom listing (fallback route)
                $url = url($slug);
                $isActive = request()->is($slug);
            } elseif (in_array($slug, $staticRoutes)) {
                // Static route with .home
                $routeName = $slug . '.home';
                $url = route($routeName);
                $isActive = request()->routeIs($routeName);
            } else {
                // Dynamic listing
                $url = route('listing.view', ['type' => $slug, 'view' => 'grid']);
                $isActive = request()->routeIs('listing.view') && request()->type == $slug;
            }
        @endphp
    ...
    @if ($menu_items->count() > 2)
        <li class="have-sub-menu">
            ...
            <ul class="Abhishek first-sub-menu">
                @foreach ($menu_items as $index => $item)
                    @if ($index >= 2)
                        @php
                            $slug = strtolower($item->slug);
    ...
    <li><a href="{{ route('pricing') }}" class="first-a {{ request()->routeIs('pricing') ? 'active' : '' }}">{{ get_phrase('Pricing') }}</a></li>
                            ...
                            <div class="header-buttons d-flex align-items-center">
                                ...
                                @elseif(user('role') == 2)
                                    <div class="dropdown at-user-dropdown">
                                        <button class="btn user-dropdown-toggle dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                           <img src="{{user('image')? asset('uploads/users/' . user('image')): (user('avatar')? user('avatar'): asset('image/user.jpg')) }}" alt="">
                                        </button>
                                        <div class="dropdown-menu user-dropdown-menu">
                                            <ul class="user-dropdown-group abhishek">
                                               ...
                                               <!---abhishek-editing---->
                                                ...
                                                <li class="sidebar-nav-item">
                                                        <a href="https://www.listify.asia/customer/wishlist" class="user-dropdown-item fill-none">
                                                            <span class="d-flex align-items-center mt-1px gap-6px">
                                                                ...
                                                                <span class="mt-1px">{{ get_phrase('My Dashboard') }}</span>
                                                            </span>
                                                        </a>
                                                  </li>
                                                ...
                                                @endif
                                                <li class="sidebar-nav-item d-none">
                                                ...
                                                <li>
                                                    <a class="user-dropdown-item fill-none" href="{{ route('user.account') }}">
                                                        ...
                                                        <span class="mt-2px">{{ get_phrase('My Account') }}</span>
                                                    </a>
                                                </li>
