...
<div class="ol-card mt-3">
    ...
    @foreach ($subscriptions as $subscription) 
        @php
            $packagetitle = App\Models\Pricing::where('id', $subscription->package_id)->value('name');
        @endphp
        
        @php
            $username = App\Models\User::where('id', $subscription->user_id)->value('name');
        @endphp      
        <tr>
            <td> {{$num++}} </td>
            <td> {{$username ?? ''}} </td>
            ...
            <td> 
                <div class="dropdown ol-icon-dropdown">
                    ...
                    <ul class="dropdown-menu">
                      <li>
                        <a class="dropdown-item fs-14px" onclick="delete_modal('{{route('admin.subscription.delete',['id'=>$subscription->id])}}')" href="javascript:void(0);" > 
                            {{get_phrase('Delete')}} 
                        </a>
                      </li>
                    </ul>
