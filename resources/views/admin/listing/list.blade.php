...
<div class="ol-card mt-3">
    ...
    @php
        ...
        // ===== CATEGORY SAFE CHECK =====
        $categoryData = App\Models\Category::where('id', $listing->category)->first();
        $category = $categoryData->name ?? 'N/A';

        $image = json_decode($listing->image)[0] ?? 0;

        // ===== BRAND & MODEL SAFE CHECK =====
        $brand = 'N/A';
        $model = 'N/A';

        if($type == 'car'){

            $brandData = App\Models\Amenities::where('id', $listing->brand)->first();
            $modelData = App\Models\Amenities::where('id', $listing->model)->first();

            $brand = $brandData->name ?? 'N/A';
            $model = $modelData->name ?? 'N/A';
        }
    ...
    <tr>
        ...
        <td>
            <img src="{{get_all_image('listing-images/'.$image)}}"
                 width="50"
                 height="50"
                 class="rounded"
                 alt="">
        ...
        <td>
            ...
            <svg data-bs-toggle="tooltip" 
                ...
                <linearGradient id="paint0_linear_16_1334"
                ...
                <ul class="dropdown-menu">
                    ...
                    <li>
                        <a class="dropdown-item fs-14px"
                           href="{{route('listing.details',['id'=>$listing->id,'type'=>$listing->type, 'slug'=>slugify($listing->title)])}}"
                           target="_blank">

                           {{get_phrase('View frontend')}}
                    ...
                    <li>
                        <a class="dropdown-item fs-14px"
                           href="{{route('admin.listing.status',['type'=>$type, 'id'=>$listing->id, 'status'=>$listing->visibility])}}">

                           {{get_phrase('Change visibility')}}
                    ...
                    <li>
                        <a class="dropdown-item fs-14px"
                           href="{{route('admin.listing.edit',['type'=>$type, 'id'=>$listing->id, 'tab'=>0])}}">
                    ...
                    <li>
                        <a class="dropdown-item fs-14px"
                           onclick="delete_modal('{{route('admin.listing.delete',['type'=>$type, 'id'=>$listing->id])}}')"
