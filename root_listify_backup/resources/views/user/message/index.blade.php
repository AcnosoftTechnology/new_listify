                    ...
                    <div class="ca-content-card">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="mt-4">
                                    @if(isset($thread_details) && $thread_details)
                                    @if (count($messages) > 0)    
                                    <ul class="message-body" id="message-container">
                                        @foreach ($messages as $message)
                                        @if ($message->sender != user('id'))    
                                            <li class="circle-img-50pxs w-100 d-flex align-items-center my-2">
                                                @if($thread_details->message_to_receiver->id == user('id'))
                                                <img src="{{ get_user_image('users/' . $thread_details->message_to_sender->image) }}" width="50px" height="50px" alt="">
                                                @else
                                                <img src="{{ get_user_image('users/' . $thread_details->message_to_receiver->image) }}" width="50px" height="50px" alt="">
                                                @endif  

                                                <div class="ps-2">
                                                    <!-- <span class="message-card in-title-14px mb-2"> {{$message->message}} </span> -->
                                                     <span class="message-card in-title-14px mb-2">
                                                        @php
                                                            $extension = strtolower(pathinfo($message->message, PATHINFO_EXTENSION));
                                                            $imageExtensions = ['jpg', 'jpeg', 'png', 'webp'];
                                                        @endphp

                                                        @if (in_array($extension, $imageExtensions))
                                                            <a href="{{ asset($message->message) }}" target="_blank">
                                    ...
                                    <form action="{{route('user.message.send',['prefix'=>$user_prefix, 'code'=>$code])}}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <div class="input-group mb-3 pt-3 w-100" style="gap:5px:">
                                            <input type="text" class="form-control w-100" placeholder="{{get_phrase('Write your message')}}" name="message">
                                            <input type="file" class="form-control mt-2" name="attachment" accept=".jpg,.jpeg,.png,.webp">
