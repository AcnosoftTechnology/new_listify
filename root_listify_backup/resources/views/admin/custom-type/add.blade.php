<form action="{{route('admin.custom-type.store')}}" method="post" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        <label class="form-label ol-form-label"> {{ get_phrase('Featured Listing') }} </label><br>
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="featured" id="active" value="1" checked>
            <label class="form-check-label" for="active">{{ get_phrase('Active') }}</label>
        </div>
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="featured" id="inactive" value="0">
            <label class="form-check-label" for="inactive">{{ get_phrase('Inactive') }}</label>
        </div>
    </div>
