...
@php
    ...
    $car_types = App\Models\Amenities::where('type', 'car')
        ->where('identifier', 'car_type')
        ->get();

    $models = App\Models\Amenities::where('type', 'car')
        ->where('identifier', 'model')
        ->get();

    $brands = App\Models\Amenities::where('type', 'car')
        ->where('identifier', 'brand')
        ->get();

    $colors = App\Models\Amenities::where('type', 'car')
        ->where('identifier', 'exterior_color')
        ->get();
    ...
    //dump([
        //'car_types_count' => $car_types->count(),
    ...
    <section>
        <div class="container">
            ...
            <div class="mixitup row gx-28px gy-30px mb-30px">
                <!-- Single Card -->
                @foreach (sort_listings_by_distance($top_listings) as $listing)
                    @php
    ...
    <section>
        <div class="container">
            ...
            <div class="row mb-30px g-28px">
                ...
                @php
                @endphp
                @foreach (sort_listings_by_distance($top_listings->where('is_popular', 'featured'), 4) as $listing)
                    @php
                        $is_in_wishlist = check_wishlist_status($listing->id, $listing->type);
                        $claimStatus = App\Models\ClaimedListing::where('listing_id', $listing->id)->where('listing_type', 'car')->first(); 
                    @endphp
    ...
    <section>
        <div class="container">
            ...
            <div class="row mb-30px g-28px">
                ...
                @php
                @endphp
                @foreach (sort_listings_by_distance($top_listings->where('is_popular', 'top'), 4) as $listing)
                    @php
                        $is_in_wishlist = check_wishlist_status($listing->id, $listing->type);
                        $claimStatus = App\Models\ClaimedListing::where('listing_id', $listing->id)->where('listing_type', 'car')->first();  
                    @endphp
