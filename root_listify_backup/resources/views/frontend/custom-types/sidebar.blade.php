...
<form id="filter-form" action="{{ route('ListingsFilter') }}" method="get" class="sidebar-accordion-area">
    ...
    <li class="sidebar-accordion-li">
        ...
        <ul class="sidebar-accordion-menu" id="countriesList">
           @foreach ($countries as $index => $countryName)
    @php
        $country = App\Models\Country::find($countryName->country);
    @endphp
    ...
    @if($country)
        @php
            ...
            $countryCount = App\Models\CustomListings::where('country', $country->id)
                ->where('type', $type)
        ...
        <li class="{{ $index >= 6 ? 'hidden' : '' }}">
            <a href="{{ route('ListingsFilter') . '?' . http_build_query($params) }}"
               class="{{ (isset($country_type) && $country->id == $country_type) ? 'active' : '' }}">
                <span>{{ $country->name }}</span>
...
@endforeach
    ...
    @if($city)
        @php
            ...
            $cityCount = App\Models\CustomListings::where('city', $city->id)
                ->where('type', $type)
                ->where('visibility', 'visible')
                ->count();
        @endphp
        ...
        <li class="{{ $index >= 6 ? 'hidden' : '' }}">
            <a href="{{ route('ListingsFilter') . '?' . http_build_query($params) }}"
               class="{{ (isset($city_type) && $city->id == $city_type) ? 'active' : '' }}">
                <span>{{ $city->name }}</span>
