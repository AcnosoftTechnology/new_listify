...
public function CompanyLogo(){
 ...
 public function Deletecompanylogo($id)
 {
  ...
  public function review_delete($id)
  {
    ...
    public function Customlisting_setting() {
        $all_customlisting = CustomlistingSetting::all();
        return view('admin.setting.customlisting_setting',compact('all_customlisting'));
    }
  
  
    public function addcustomlisting() {      
     $customTypes = CustomType::select('name')->whereNotIn('slug', ['hotel', 'car', 'Beauty', 'Restaurant', 'Real-Estate'])->get();
     return view('admin.setting.customlisting_setting.add', compact('customTypes'));
   }
  
  
  public function storecustomlisting(Request $request){
    
    $request->validate([
        'listing_type'        => 'nullable|string|max:250',
        'banner_title'        => 'nullable|string|max:250',
        'banner_description'  => 'nullable|string',
        'banner_bg_image'     => 'nullable|image|mimes:jpeg,png,jpg,gif,svg,webp|max:2048',
        'banner_tab_name'     => 'nullable|string|max:250',
        'banner_tab_link'     => 'nullable|string|max:250',
        'cta_bg_image'        => 'nullable|image|mimes:jpeg,png,jpg,gif,svg,webp|max:2048',
        'cta_bg_title'        => 'nullable|string|max:250',
        'cta_bg_description'  => 'nullable|string',
        'cta_tab_name'        => 'nullable|string|max:250',
        'cta_tab_link'        => 'nullable|string|max:250',
    ]);
    ...
    if ($request->hasFile('banner_bg_image')) {
        ...
        $bannerName = time() . '_banner.' . $bannerImage->getClientOriginalExtension();
        $bannerImage->move(public_path('uploads/customlisting-banner'), $bannerName);
        $bannerImagePath = 'uploads/customlisting-banner/' . $bannerName;
    }

    // CTA image upload
    if ($request->hasFile('cta_bg_image')) {
        $ctaImage = $request->file('cta_bg_image');
        $ctaName = time() . '_cta.' . $ctaImage->getClientOriginalExtension();
        $ctaImage->move(public_path('uploads/customlisting-banner'), $ctaName);
        $ctaImagePath = 'uploads/customlisting-banner/' . $ctaName;
    }

    CustomlistingSetting::create([

        'listing_type'        => $request->listing_type ?? '',
        'slug' => $request->listing_type ? Str::slug($request->listing_type): '',
        'banner_title'        => $request->banner_title ?? '',
        'banner_description'  => $request->banner_description ?? '',
        'banner_bg_image'     => $bannerImagePath ?? '',
        'banner_tab_name'     => $request->banner_tab_name ?? '',
        'banner_tab_link'     => $request->banner_tab_link ?? '',
        'cta_bg_image'        => $ctaImagePath ?? '',
        'cta_bg_title'        => $request->cta_bg_title ?? '',
        'cta_bg_description'  => $request->cta_bg_description ?? '',
        'cta_tab_name'        => $request->cta_tab_name ?? '',
        'cta_tab_link'        => $request->cta_tab_link ?? '',
        'created_at'          => now(),

    ]);
    
   
    if($request->seo_on_page == 1){
        \App\Models\SeoField::create([
            'route'       => $request->listing_type,
            'created_at'  => now(),
        ]);
    }

    Toastr::success('Custom Listing added successfully!');
    return redirect()->route('admin.Customlisting.settings');
 }

  
    public function editcustomlisting($id){

        $editcustlisting = CustomlistingSetting::findOrFail($id);

        return view('admin.setting.customlisting_setting.edit',compact('editcustlisting'));
    }

  
public function updatecustomlisting(Request $request, $id){
    ...
    // Record find karo
    $listing = CustomlistingSetting::findOrFail($id);
    ...
    if ($request->hasFile('banner_bg_image')) {
        ...
        $bannerName = time() . '_banner.' . $bannerImage->getClientOriginalExtension();
        $bannerImage->move(public_path('uploads/customlisting-banner'), $bannerName);
        $listing->banner_bg_image = 'uploads/customlisting-banner/' . $bannerName;
    }
    ...
    if ($request->hasFile('cta_bg_image')) {
        ...
        $ctaName = time() . '_cta.' . $ctaImage->getClientOriginalExtension();
        $ctaImage->move(public_path('uploads/customlisting-banner'), $ctaName);
        $listing->cta_bg_image = 'uploads/customlisting-banner/' . $ctaName;
    }

    // Baaki fields update karo
    $listing->listing_type       = $request->listing_type ?? '';
    $listing->slug = $request->listing_type ? Str::slug($request->listing_type): '';
    $listing->banner_title       = $request->banner_title ?? '';
    $listing->banner_description = $request->banner_description ?? '';
    $listing->banner_tab_name    = $request->banner_tab_name ?? '';
    $listing->banner_tab_link    = $request->banner_tab_link ?? '';
    $listing->cta_bg_title       = $request->cta_bg_title ?? '';
    $listing->cta_bg_description = $request->cta_bg_description ?? '';
    $listing->cta_tab_name       = $request->cta_tab_name ?? '';
    $listing->cta_tab_link       = $request->cta_tab_link ?? '';

    $listing->save();

    Toastr::success('Custom Listing updated successfully!');
    return redirect()->route('admin.Customlisting.settings');
}
...
public function destroy($id){
  ...
  $listing = CustomlistingSetting::findOrFail($id);
    ...
    if ($listing->banner_bg_image && file_exists(public_path($listing->banner_bg_image))) {
        unlink(public_path($listing->banner_bg_image));
    }

    if ($listing->cta_bg_image && file_exists(public_path($listing->cta_bg_image))) {
        unlink(public_path($listing->cta_bg_image));
    }

    $listing->delete();

    Toastr::success('Custom Listing deleted successfully!');
    return redirect()->route('admin.Customlisting.settings');
}
