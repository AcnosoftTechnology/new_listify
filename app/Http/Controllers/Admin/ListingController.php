...
class ListingController extends Controller{
    ...
    public function listing_store(Request $request, $type){
        ...
        $coordinates = GeolocationService::resolveListingCoordinates(
            $request->latitude,
            $request->longitude,
            $request->address,
            $request->city,
        ...
        if($type == 'car'){
            ...
            $data['is_popular'] = $request->is_popular ?? 0;
          //if(!empty($data['price']) || !empty($data['bed']) || !empty($data['bath']) || !empty($data['size']) || !empty($data['dimension'])){
...
public function listing_image_delete(Request $request, $type, $id){
  
    $image = basename($request->image);

    $listing = match($type) {
        'car' => CarListing::find($id),
        'beauty' => BeautyListing::find($id),
        'hotel' => HotelListing::find($id),
        'real-estate' => RealEstateListing::find($id),
        'restaurant' => RestaurantListing::find($id),
        default => CustomListings::find($id),
    };

    $imageArray = json_decode($listing->image, true);
    ...
    foreach ($imageArray as $key => $img) {
        ...
        if (basename($img) == $image) {
            ...
            $listing->image = json_encode(array_values($imageArray));
            $listing->save();

            $filePath = public_path('uploads/listing-images/'.$image);
