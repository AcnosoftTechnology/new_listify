...
public function form_builder(Request $request)
{
    ...
    $custom_types = CustomType::select('name', 'slug')->get();
...
public function formBuilderCreate() {
    $customTypes = CustomType::all();
    return view("admin.form-builder.create", compact('customTypes'));
}
    ...
    public function builderAppointment(Request $request)
    {
       ...
       // $appointment->form_builder = json_encode($formBuilderData);
        ...
        // SAVE EXTRA FORM DATA HERE
        $appointment->aditional_information = json_encode($formBuilderData);
